...
onMount( () =>{
    ...
    window.$.fn.easeScroll = function(options) {
    ! function() {
        function e() {
            var e = !1;
...
window.$("html").easeScroll({
  ...
  //Mobile Menu
	...
	//Make sure menu is closed before page change.
  ...
  toggle.checked = false;
	...
	(() => {
		...
		//console.log('navigation wrap section');
		let refOffset = 0;
		const navigationHeight = 45;
		const navigation = document.querySelector('.nav');
		//const navigationInner = document.querySelector('.menu');
		const menuToggle = document.getElementById('toggle');
		const handler = () => {
		const newOffset = window.scrollY || window.pageYOffset;

			if (newOffset > navigationHeight) {
				if (newOffset > refOffset) {
					navigation.classList.remove('animateIn');
					navigation.classList.add('animateOut');
					menuToggle.checked = false;
				} else {
					navigation.classList.remove('animateOut');
					navigation.classList.add('animateIn');
				}
